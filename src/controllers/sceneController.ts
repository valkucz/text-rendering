...
export class SceneController implements Controller {
    ...
    constructor(id: string, object: Glyph){
        this.rotateX = document.getElementById(id + '-rotate-x') as HTMLInputElement;
        this.rotateY = document.getElementById(id + '-rotate-y') as HTMLInputElement;
        this.rotateZ = document.getElementById(id + '-rotate-z') as HTMLInputElement;
    ...
    addEventListener(app: App): void {
        ...
        this.moveX.addEventListener('input', () => {
            this.object.move(vec3.fromValues(parseInt(this.moveX.value) - this.moveXvalue, 0, 0));
            this.moveXvalue = parseInt(this.moveX.value);
            app.notify();
        });
        this.moveY.addEventListener('input', () => {
            this.object.move(vec3.fromValues(0, parseInt(this.moveY.value) - this.moveYvalue, 0));
            this.moveYvalue = parseInt(this.moveY.value);
            app.notify();
        });
        this.moveZ.addEventListener('input', () => {
            this.object.move(vec3.fromValues(0, 0, parseInt(this.moveZ.value) - this.moveZvalue));
            this.moveZvalue = parseInt(this.moveZ.value);
            app.notify();
        });
